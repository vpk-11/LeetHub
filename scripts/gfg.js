    ...
    submitBtn.addEventListener('click', function () {
      ...
      const submission = setInterval(() => {
        ...
        if (output.includes('Problem Solved Successfully') && START_MONITOR) {
          ...
          // format data
          // GFG/ top-level prefix keeps GeeksforGeeks submissions separate from LeetCode's
          // own LeetCode/ prefix (see buildProblemPath in scripts/leetcode/util.js) - a
          // deliberate addition on top of 3.0, which has no GFG support at all.
          const probName = `GFG/${title} - GFG`;
