...
/** The `question` sub-object of GraphQL `submissionDetails`, with `questionFrontendId`
 * merged in afterward from the separate `questionDetail` query (init() does this itself -
 * LeetCode's submissionDetails query doesn't return it directly). */
...
/** GraphQL `submissionDetails` response shape - see the query body in init(). */
...
class LeetCodeV1 {
  ...
  // V1 never populates this (no GraphQL submission-details fetch on the old UI) - always
  // undefined at runtime, same as before typing. Declared so callers that accept either
  // version through one union type (loader() in leetcode.ts) can optional-chain through it
  // uniformly instead of needing a type narrow per call site.
  submissionData?: SubmissionDetails;
  progressSpinnerElementId = 'leethub_progress_elem';
  progressSpinnerElementClass = 'leethub_progress';
  ...
  findCode(commitMsg?: string): Promise<string | undefined> | undefined {
    ...
    if (checkElem(e)) {
      ...
      const submissionRef = document.getElementById('result-state') as HTMLAnchorElement | null;
    ...
    return fetch(submissionURL)
      ...
      .then(responseText => {
        ...
        for (let i = 0; i < scripts.length; i++) {
          const text = (scripts[i] as HTMLScriptElement).innerText;
  ...
  /** @returns a language file extension, e.g. ".py" */
  getLanguageExtension(): string | null {
    const tag = [
    ...
    if (tag && tag.length > 0) {
      for (let i = 0; i < tag.length; i += 1) {
        ...
        if (elem !== undefined && elem !== null && languages[elem] !== undefined) {
  ...
  /** @returns the submission's verbose language name, e.g. "Python3" - forward-sourced,
    ...
    if (
      ...
      if (notesdiv) {
        for (let i = 0; i < notesdiv.childNodes.length; i++) {
          ...
          const text = (notesdiv.childNodes[i].childNodes[0] as HTMLElement).innerText;
          if (text) {
            notes = `${notes}\n${text.trim()}`.trim();
          }
        }
  ...
  getProblemNameSlug(): string {
    ...
    if (checkElem(questionElem)) {
      ...
      if (checkElem(qtitle)) {
        questionTitle = (qtitle[0] as HTMLElement).innerText;
      }
    }
  ...
  getSuccessStateAndUpdate(): boolean {
    ...
    // check success state for a explore section problem
      ...
      (resultState as HTMLElement).innerText === 'Accepted'
    ) {
  ...
  parseQuestion(): string | undefined {
    ...
    if (checkElem(questionElem)) {
      ...
      // Problem title.
      let qtitle: string;
      const qtitleElem = document.getElementsByClassName('css-v3d350');
      if (checkElem(qtitleElem)) {
        qtitle = qtitleElem[0].innerHTML;
      } else {
      ...
      return `<h2><a href="${questionUrl}">${qtitle}</a></h2><h3>${this.difficulty}</h3><hr>${qbody}`;
    } else if (checkElem(questionDescriptionElem)) {
      let questionTitle: string;
      const questionTitleElem = document.getElementsByClassName('question-title');
      if (checkElem(questionTitleElem)) {
        questionTitle = (questionTitleElem[0] as HTMLElement).innerText;
      } else {
        questionTitle = 'unknown-problem';
      }

      const questionBody = questionDescriptionElem[0].innerHTML;
      return `<h2>${questionTitle}</h2><hr>${questionBody}`;
    }
    return undefined;
  }
  ...
  startSpinner(): void {
    try {
      ...
      if (!elem) {
        ...
        elem.style.cssText = 'margin-right: 20px;padding-top: 2px;';
      }
  ...
  insertToAnchorElement(elem: HTMLElement): void {
    if (document.URL.startsWith('https://leetcode.com/explore/')) {
      ...
      if (
        ...
        if (target.childNodes.length > 0) target.childNodes[0].parentElement?.prepend(elem);
      }
    } else {
  ...
  /* Creates a tick mark before "Run Code" button signaling LeetHub has done its job */
  markUploaded(): void {
...
class LeetCodeV2 {
  submissionData?: SubmissionDetails;
  // Set two different ways at runtime: a number from the interceptor's parsed submission
  // response (see leetcode.ts's listenForAutoSubmit), or a string from submitBtn.ts's
  // URL-regex fallback when viewing an already-accepted submission with no fresh /submit/
  // this session. Both are accepted here rather than picking one and silently narrowing
  // the other's real behavior.
  submissionId?: string | number;
  difficulty?: string;
  progressSpinnerElementId = 'leethub_progress_elem';
  progressSpinnerElementClass = 'leethub_progress';
  ...
  async init(): Promise<void> {
    ...
    const submissionData: SubmissionDetails = await fetch(
  ...
  getCode(): string | null {
    ...
    return (code[0] as HTMLElement).innerText;
  }
  ...
  getLanguageExtension(): string {
    ...
    const tag = document.querySelector('button[id^="headlessui-listbox-button"]') as HTMLElement;
    if (!tag) {
  ...
  parseStats(): string | null {
    if (this.submissionData != null) {
      ...
      return formatStats(
        ...
        String(spacePercentile)
      );
    }

    const probStats = (document.getElementsByClassName('flex w-full pb-4')[0] as HTMLElement)
      .innerText.split('\n');
    if (!checkElem(probStats)) {
      return null;
    }
  ...
  parseQuestionTitle(): string {
    if (this.submissionData != null) {
      return this.submissionData.question.title;
    }

    let questionTitle = (document.getElementsByTagName('title')[0] as HTMLElement).innerText
      .split(' ')
      .slice(0, -2)
      .join(' ');
  ...
  parseQuestionDescription(): string | null {
    if (this.submissionData != null) {
    ...
    return (description[0] as HTMLMetaElement).content;
  }
  ...
  parseDifficulty(): string {
    ...
    if (checkElem(diffElement)) {
      return (diffElement[0].children[0] as HTMLElement).innerText;
    }
...
export type { SubmissionDetails, SubmissionQuestion, TopicTag, LangInfo };
